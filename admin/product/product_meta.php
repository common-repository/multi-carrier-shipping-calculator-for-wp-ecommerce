...
function wpec_auctioninc_meta_box_callback($post) {
    ...
    echo '<tr>';
    echo '<th>';
    echo '<label for="auctioninc_lot_size">' . __('Lot Size', 'wpec_auctioninc') . '</label>';
    echo '</th>';
    echo '<td>';
    echo '<input type="text" name="auctioninc_lot_size" id="auctioninc_lot_size" value="' . esc_attr($lot_size) . '" placeholder="1">';
    echo '<p class="description">' . __('Number of separate boxes required to ship this item.  When setting lot size > 1, set the weight/dims to the average across the boxes and set Package flag to "Separately".', 'wpec_auctioninc') . '</p>';
    echo '</td>';
    echo '</tr>';
    ...
    'USPPP' => __('USPS Retail Ground', 'wpec_auctioninc'),
